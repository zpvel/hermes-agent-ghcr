...
def _resolve_unique_custom_provider_model(
    raw_input: str,
    custom_providers: list | None = None,
) -> Optional[tuple[str, str]]:
    """Resolve an exact model ID to a saved custom provider when unique.

    This supports a common gateway workflow where the active provider is the
    generic ``custom`` endpoint, but the user wants to jump to one of their
    named ``custom_providers`` rows by model ID alone, e.g. ``/model gpt-5.4``.

    Returns ``(provider_slug, model_id)`` when exactly one saved custom
    provider exposes that model. Returns ``None`` when there is no match or the
    match would be ambiguous across multiple saved custom providers.
    """
    key = (raw_input or "").strip().lower()
    if not key or not isinstance(custom_providers, list):
        return None
    ...
    for entry in custom_providers:
        ...
        if not display_name or not model_id or not api_url:
            continue
        if model_id.lower() != key:
            continue

        pair = (custom_provider_slug(display_name), model_id)
        if pair in seen:
            continue
        seen.add(pair)
        matches.append(pair)
...
def switch_model(
    ...
    # =================================================================
        ...
        if alias_result is not None:
            ...
            key = raw_input.strip().lower()
            custom_match = None
            if _current_is_custom_family:
                custom_match = _resolve_unique_custom_provider_model(
                    raw_input,
                    custom_providers,
                )
                if custom_match is not None:
                    target_provider, new_model = custom_match
                    logger.debug(
                        "Resolved model '%s' to saved custom provider %s before alias fallback",
                        raw_input,
                        target_provider,
                    )

            if custom_match is None and key in MODEL_ALIASES:
        ...
        # --- Step e: prefer an exact saved custom-provider model match ---
        # This runs before generic provider detection so user-defined custom
        # endpoints win over broad catalog heuristics like routing GPT model
        # names to Codex OAuth.
        if target_provider == current_provider and not resolved_alias:
            custom_match = _resolve_unique_custom_provider_model(
                new_model,
                custom_providers,
            )
            if custom_match is not None:
                target_provider, new_model = custom_match
                logger.debug(
                    "Resolved model '%s' to saved custom provider %s",
                    raw_input,
                    target_provider,
                )

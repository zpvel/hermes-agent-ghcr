...
class GatewayStreamConsumer:
    ...
    async def rewrite_final_response(self, text: str) -> bool:
        """Replace the visible final reply text after streaming completes."""
        text = self._clean_for_display(text)
        if not text.strip():
            return False

        if self._message_id or not self._already_sent:
        ...
        if self._fallback_final_send:
            ...
            return bool(self._final_response_sent)

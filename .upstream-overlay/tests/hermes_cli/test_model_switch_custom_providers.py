...
def test_switch_model_auto_routes_unique_custom_provider_from_generic_custom(monkeypatch):
    """Bare model IDs should auto-route to a unique saved custom provider."""
    captured = {}

    def _resolve_runtime_provider(requested):
    ...
    result = switch_model(
        ...
        current_base_url="https://v2.aicodee.com/v1",
        current_api_key="sk-main",
        user_providers={},
        custom_providers=[
            {
                "name": "openclaw-gpt",
    ...
    assert result.success is True
    assert result.target_provider == "custom:openclaw-gpt"
    assert result.provider_label == "openclaw-gpt"
    assert result.new_model == "gpt-5.4"
    assert result.base_url == "https://tokenx24.com/v1"
    assert result.api_key == "sk-openclaw"
    assert captured["requested"] == "custom:openclaw-gpt"


def test_switch_model_prefers_unique_custom_provider_before_alias_fallback(monkeypatch):
    """Custom-family providers should not let GPT aliases fall through to Codex."""
    captured = {}
    ...
    result = switch_model(
        raw_input="gpt-5.4",
        current_provider="custom:openclaw-gpt",
        current_model="gpt-5.4",
        current_base_url="https://tokenx24.com/v1",
        current_api_key="",
        user_providers={},
        custom_providers=[
            {
...
def test_switch_model_prefers_unique_custom_provider_over_codex_detection(monkeypatch):
    """Exact saved custom-provider matches should win over generic Codex detection."""
    captured = {}
    ...
    result = switch_model(
        ...
        current_model="claude-sonnet-4-6-20251001",
        current_base_url="https://api.anthropic.com",
        current_api_key="sk-anthropic",
        user_providers={},
        custom_providers=[
            {
                "name": "openclaw-gpt",

...
class AIAgent:
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        if self.provider == "gemini":
            ...
            if is_native_gemini_base_url(str(client_kwargs.get("base_url", ""))):
                safe_kwargs = {
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
                     ...
                     recent_user_image_paths: Optional[List[str]] = None) -> str:
        ...
        """
        function_args = self._augment_tool_args_with_chat_media(
            function_name,
            function_args,
            recent_user_image_paths,
        )
    ...
    @staticmethod
    def _extract_recent_user_image_paths(messages: Optional[List[Dict[str, Any]]]) -> List[str]:
        """Return image paths hinted in the most recent user message that has them."""
        ...
        patterns = (
            re.compile(r"image_url:\s*(.+?)(?:\s*~\]|\]|$)", re.IGNORECASE),
            re.compile(r"\[User sent an image:\s*(.+?)\]", re.IGNORECASE),
        )
        ...
        for msg in reversed(messages or []):
            ...
            for pattern in patterns:
                for match in pattern.finditer(text):
                    ...
                    if not candidate:
                        continue
                    if candidate.startswith(("http://", "https://", "data:image/", "/")) or re.match(r"^[A-Za-z]:[\\/]", candidate):
    ...
    @staticmethod
    def _augment_tool_args_with_chat_media(
        function_name: str,
        function_args: dict,
        recent_user_image_paths: Optional[List[str]] = None,
    ) -> dict:
        """Attach recent chat images for tools that can use them implicitly."""
        if function_name != "image_generate":
            return function_args
        if not recent_user_image_paths:
            return function_args
        if (function_args or {}).get("reference_images"):
            return function_args

        augmented_args = dict(function_args or {})
        augmented_args["_recent_user_image_paths"] = list(recent_user_image_paths)
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        recent_user_image_paths = self._extract_recent_user_image_paths(messages)
        for tool_call in tool_calls:
        ...
        def _run_tool(index, tool_call, function_name, function_args):
            ...
            try:
                result = self._invoke_tool(
                    ...
                    effective_task_id,
                    tool_call.id,
                    recent_user_image_paths=recent_user_image_paths,
                )
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if _block_msg is not None:
                ...
                try:
                    tool_function_args = self._augment_tool_args_with_chat_media(
                        function_name,
                        function_args,
                        recent_user_image_paths,
                    )
                    function_result = handle_function_call(
                        function_name, tool_function_args, effective_task_id,
